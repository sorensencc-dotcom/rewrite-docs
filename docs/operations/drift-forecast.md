...
3. **RL vault sync.** ✅ Resolved 2026-07-04: source = GitHub `sorensencc-dotcom/rewrite-mcp` (main); first sync complete (14 docs via `scripts/rl-vault-sync.js` + `rl-vault-manifest.json`). Residual risk: sync is manual until wired into weekly automation; rl-ref staleness now tracked via `rl-ref/index.md` last-sync stamp.
