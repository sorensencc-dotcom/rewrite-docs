...
3. **RL vault first sync pending.** ~~Source unconfirmed~~ → resolved 2026-07-03: GitHub `sorensencc-dotcom/rewrite-mcp` (main). `rl-ref/` still empty until sync patterns are chosen and a first test sync runs ([RL-VAULT-SETUP](../rewrite-labs/RL-VAULT-SETUP.md)).
