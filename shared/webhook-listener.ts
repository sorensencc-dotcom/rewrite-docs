...
export interface SLOEvent {
  type: 'VERTICAL_DRIFT' | 'SPA_HYDRATION_FAILURE' | 'CONFIDENCE_DROP' | 'TIMEOUT' | 'SCHEMA_MISMATCH';
  adapter: string;
  severity: 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW';
  timestamp: number;
  details?: any;
...
export function createWebhookRouter() {
  ...
  router.post('/slo/violation', async (req: Request, res: Response) => {
    const event: SLOViolationEvent = req.body;
...
async function handleSLOViolation(event: SLOViolationEvent): Promise<void> {
...
async function handleSLOEvent(event: SLOEvent): Promise<void> {
...
async function handleDriftEvent(event: SLOEvent): Promise<void> {
...
async function handleHydrationFailure(event: SLOEvent): Promise<void> {
...
async function handleConfidenceDrop(event: SLOEvent): Promise<void> {
...
async function handleTimeout(event: SLOEvent): Promise<void> {
...
async function handleSchemaMismatch(event: SLOEvent): Promise<void> {
...
async function notifyOncall(event: SLOViolationEvent | SLOEvent): Promise<void> {
...
async function notifySlack(event: SLOViolationEvent | SLOEvent): Promise<void> {
...
async function logEvent(event: SLOViolationEvent | SLOEvent): Promise<void> {
