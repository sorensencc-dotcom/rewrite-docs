...
export class PromMetricsRegistry {
    failureEventsCounter;
    repairAttemptsCounter;
    escalationsCounter;
    manualInterventionsCounter;
    nodeRetriesCounter;
    buildRetriesCounter;
    anomalyScoresHistogram;
    constructor() {
    ...
    get failureEvents() {
        return {
            inc: (labels, value) => this.failureEventsCounter.inc(labels, value),
        };
    }
    get repairAttempts() {
        return {
            inc: (labels, value) => this.repairAttemptsCounter.inc(labels, value),
        };
    }
    get escalations() {
        return {
            inc: (labels, value) => this.escalationsCounter.inc(labels, value),
        };
    }
    get manualInterventions() {
        return {
            inc: (labels, value) => this.manualInterventionsCounter.inc(labels, value),
        };
    }
    get nodeRetries() {
        return {
            inc: (labels, value) => this.nodeRetriesCounter.inc(labels, value),
        };
    }
    get buildRetries() {
        return {
            inc: (labels, value) => this.buildRetriesCounter.inc(labels, value),
        };
    }
    get anomalyScores() {
        return {
            observe: (labels, value) => this.anomalyScoresHistogram.observe(labels, value),
        };
    }
}
