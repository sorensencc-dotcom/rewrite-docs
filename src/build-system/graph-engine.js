...
export class BuildGraphEngine {
    ...
    constructor(graph) {
        ...
        this.orchestrator = new SelfHealingOrchestrator(new FailureDetector({ timeoutThresholdFactor: 1.0, anomalyScoreWeights: { durationMs: 0.5 } }), new AutoRestartEngine({ maxNodeRetries: 3, maxBuildRetries: 5, baseDelayMs: 200, backoffFactor: 2.0 }), new AutoRepairEngine(), new StateRecoveryManager(new InMemoryStateStore()), { emit: async () => { } }, // dummy event sink
        new NoopMetricsRegistry(), { anomalyThreshold: 80 }, new ConsoleStateMachineLogger());
    }
    getSelfHealingOrchestrator() {
    ...
    async executeNode(nodeId, build_id, provenance) {
        return this.executeNodeWithSelfHealing(nodeId, build_id, provenance);
    }
    async executeNodeWithSelfHealing(nodeId, build_id, provenance) {
        const node = this.graph.nodes.find((n) => n.id === nodeId);
        if (!node)
            return { success: false, error: new Error(`Node not found: ${nodeId}`) };
        const ctx = {
            buildId: build_id,
            nodeId: nodeId,
            nodeRetryCount: 0,
            ...
            repairHooks: {
                reduceBatchSize: async (id) => {
                    console.log(`Executing reduceBatchSize hook for node ${id}`);
                    if (node) {
                        node.parallelJobs = Math.max(1, Math.floor((node.parallelJobs || 4) / 2));
                    }
                },
                reduceConcurrency: async (id) => {
                    console.log(`Executing reduceConcurrency hook for node ${id}`);
                    if (node) {
                        node.parallelJobs = Math.max(1, Math.floor((node.parallelJobs || 4) / 2));
                    }
                },
                clearMemoryCache: async (id) => {
                    console.log(`Executing clearMemoryCache hook for node ${id}`);
                    if (node) {
                        node.memoryLimit = '4g';
                    }
                },
                useCachedArtifacts: async (id) => {
                    if (node) {
                        node.useCache = true;
                    }
                },
                clearOutputsAndRebuild: async (id) => {
                    if (node) {
                        node.cleanBuild = true;
                    }
                },
                applyPinnedDepOverrides: async (id) => {
                    if (node) {
                        node.usePinnedDependencies = true;
                    }
                },
                fallbackToCpu: async (id) => {
                    if (node) {
                        node.runtime = 'cpu';
                    }
                }
        ...
        try {
            await this.orchestrator.runNode(ctx, async () => {
                ...
                const artifact = this.lineage.recordArtifact(nodeId, Array.from(context.inputs.keys()), Array.from(context.outputs.keys()), provenance, build_id, undefined);
                ...
                await this.orchestrator.getRecovery().saveCheckpoint({ buildId: build_id, nodeId }, Array.from(this.executionContexts.entries()) // Maps are not easily JSON serializable, so convert to array
                );
            });
            return { success: true };
        }
            ...
            if (artifact) {
                this.lineage.updateArtifactStatus(artifact.artifact_id, 'failed', error);
            }
            return { success: false, error: error };
        }
    }
    async executePlan(plan, provenance) {
        const errors = [];
    ...
    getExecutionLayer(nodeId, visited) {
        const layer = [];
        const node = this.graph.nodes.find((n) => n.id === nodeId);
        if (!node)
            return layer;
