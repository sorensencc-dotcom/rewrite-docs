...
import { CICStateStore, BackendId, SLAViolation } from "../server/cicStateStore.js";
...
import {
  ...
  loadAuditLog,
  AuditLog
} from "../../governance/audit-policy.js";
import {
  executePromotion,
  executeRollback,
  PromotionRecord,
  RollbackRecord
} from "../../governance/promotion-rollback.js";
...
describe("Phase 5: Operational Playbooks, Runbooks & SLA Enforcement", () => {
  ...
  describe("SLA Violations", () => {
    test("1. Latency breach triggers SLA violation and driftSpike playbook", () => {
      ...
      expect(state.violations).toContainEqual(
        expect.objectContaining({ category: "latency_breach", severity: "SEV-3" })
      );
    ...
    test("3. Token breach triggers SLA violation and drift score penalty", () => {
      ...
      expect(state.violations).toContainEqual(
        expect.objectContaining({ category: "token_breach", severity: "SEV-3" })
      );
      expect(state.activePlaybooks.driftSpike).toBe(true);
    });
    ...
    test("4. Backlog breach triggers SLA violation and ingestionRecovery playbook", () => {
      ...
      expect(state.violations).toContainEqual(
        expect.objectContaining({ category: "ingestion_backlog", severity: "SEV-2" })
      );
    ...
    test("6. Routing oscillation triggers violation and freezes routing", () => {
      ...
      expect(state.violations).toContainEqual(
        expect.objectContaining({ category: "routing_oscillation", severity: "SEV-2" })
      );
    ...
    test("10. Unsealed audit chain break is detected", () => {
      ...
      const auditLog = JSON.parse(fs.readFileSync(tempAuditFile, "utf8")) as AuditLog;
  ...
  describe("Playbooks", () => {
    ...
    test("6. Backend Recovery Playbook resets drift scores", () => {
      ...
      for (const k in resetState.drift) {
        resetState.drift[k as BackendId] = 0;
      }
  ...
  describe("Runbooks", () => {
    ...
    test("5. Ingestion Recovery Runbook triggers SEV-2 on backlog limit", async () => {
      ...
      const entry = (ts: number) => JSON.stringify({
        backend: "ollama",
        timestamp: ts,
        type: "client_session",
  ...
  describe("Ingestion Daemon", () => {
    test("1. Daemon correctly initializes and starts/stops interval", () => {
      ...
      daemon.start();
      expect((daemon as any).intervalId).not.toBeNull();
      daemon.stop();
      expect((daemon as any).intervalId).toBeNull();
    });
