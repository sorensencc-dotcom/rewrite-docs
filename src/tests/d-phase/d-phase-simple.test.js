...
describe("D-Phase: Response Validator (Fire-Drill)", () => {
    ...
    it("D-1: detects 500 errors", async () => {
        ...
        await expect(mockProvider.callChat(null, null)).rejects.toThrow();
    });
    ...
    it("D-4: validator rejects empty text", async () => {
        const result = await mockProvider.callChat({ name: "test", provider: "mock", type: "mock", env: "", apiBase: "", supports: { chat: true, toolCalls: true, vision: true, streaming: true, embeddings: true } }, { model: "test", messages: [] });
        mockProvider.simulate({ type: "empty" });
        const emptyResult = await mockProvider.callChat({ name: "test", provider: "mock", type: "mock", env: "", apiBase: "", supports: { chat: true, toolCalls: true, vision: true, streaming: true, embeddings: true } }, { model: "test", messages: [] });
    ...
    it("D-6: validator detects capability mismatch", () => {
        ...
        const spec = { name: "text-only", provider: "test", type: "mock", env: "", apiBase: "", supports: { chat: true, toolCalls: true, vision: false, streaming: true, embeddings: true } };
    ...
    it("All 6 failure modes are detectable", async () => {
        ...
        for (const mode of modes) {
            mockProvider.simulate({ type: mode });
...
//# sourceMappingURL=d-phase-simple.test.js.map