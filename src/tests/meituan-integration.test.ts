...
describe("Meituan Provider Integration", () => {
  ...
  it("should return stub response in test mode without API key", async () => {
    ...
    expect(response.text).toBe(
      "Domestic AI chips reduce dependency on foreign accelerators and allow tighter optimization between hardware and training stack."
    );
  ...
  it("should throw on missing API key in production", async () => {
    ...
    await expect(meituanProvider.chat(req)).rejects.toThrow(
      "MEITUAN_API_KEY required"
    );
  });
