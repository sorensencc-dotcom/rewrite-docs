...
describe("C-6: No Hidden Nondeterminism", () => {
    ...
    describe("no random() calls", () => {
        it("selection not affected by Math.random()", () => {
            ...
            const selections = Array.from({ length: 200 }, () => router.selectModel(profile, payload));
    ...
    describe("no unstable JSON.stringify", () => {
        it("object key ordering deterministic", () => {
            ...
            const models = Array.from({ length: 30 }, () => router.selectModel(profile, payload));
    ...
    describe("no async race conditions", () => {
        it("100 parallel routing calls produce identical output", async () => {
            ...
            const promises = Array.from({ length: 100 }, () => Promise.resolve(router.selectModel(profile, payload)));
        ...
        it("100 parallel agent calls produce identical output", async () => {
            const agent = new OrchestratorAgent();
            const promises = Array.from({ length: 100 }, () => agent.runPlan("race test"));
            const results = await Promise.all(promises);
            const first = results[0];
            results.forEach(result => {
        ...
        it("concurrent routing under different profiles stays independent", async () => {
            ...
            const promises = profiles.flatMap(profile => Array.from({ length: 20 }, () => Promise.resolve(router.selectModel(profile, payload))));
            const results = await Promise.all(promises);
            const grouped = [
                results.slice(0, 20),
                results.slice(20, 40),
    ...
    describe("1000-run stress test", () => {
        it("1000 consecutive runs → all identical", () => {
            ...
            const results = Array.from({ length: 1000 }, () => router.selectModel(profile, payload));
...
//# sourceMappingURL=c06-no-hidden-nondeterminism.test.js.map