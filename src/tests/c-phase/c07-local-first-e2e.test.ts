...
import { CICState, BackendId } from "../../maal/router/maal-routing-policy.js";
...
describe("Local-First E2E Integration Workflow", () => {
  ...
  let worldStateBackup: string | null = null;
  let trainingAssetsBackup: string | null = null;
  let rewriteLabsAssetsBackup: string | null = null;
  ...
  it("Step 2: Ingestion — Normalizes assets and fans out to 4 pipelines", () => {
    ...
    expect(trainingAssets.some((a: any) => a.id === result.pipelines.corpus.assetIds[0])).toBe(true);
    ...
    expect(rewriteAssets.some((a: any) => a.id === result.pipelines.corpus.assetIds[0])).toBe(true);
  });
  ...
  it("Step 3: Auditing — Emits and loads deterministic traces, dashboard console view, and text reports", () => {
    ...
    const treatmentMessages = assets.map(asset => 
      bus.send("cic-ingestion", "treatment-pipeline", "evidence_item", {
        assetId: asset.id,
        type: asset.type,
    ...
    const trace = emitTrace(
      orchestrationResult,
      { docs: [docPath1, docPath2], images: [] },
      runId,
      orchestrationResult.assetCount,
      messagesByType
    );
  ...
  it("Step 4: TorqueQuery Introspection — Verifies local-first mode assertions", () => {
    ...
    const qEnabled = queries.find((q: any) => q.id === "q-local-first-enabled");
    ...
    const qSnapshot = queries.find((q: any) => q.id === "q-local-first-snapshot");
    ...
    const qCert = queries.find((q: any) => q.id === "q-local-first-certificate");
  ...
  it("Step 5: Determinism Stability — Iterative runs generate identical hashes and state delta", () => {
    const runsCount = 50;
    const runs = Array.from({ length: runsCount }, () =>
      orchestrateMultiPipeline({
        docs: [docPath1, docPath2],
        images: []
      })
    );
