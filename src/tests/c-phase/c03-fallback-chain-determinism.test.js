...
describe("C-3: Fallback Chain Determinism", () => {
    ...
    describe("primary → fallback transition", () => {
        ...
        it("fallback order stable (deterministic chain)", () => {
            ...
            const choices = Array.from({ length: 20 }, () => router.selectModel(profile, payload));
    ...
    describe("final model stability", () => {
        it("final model always same under fallback", () => {
            ...
            const finals = Array.from({ length: 100 }, () => router.selectModel(profile, payload));
        ...
        it("final provider always same under fallback", () => {
            ...
            const providers = Array.from({ length: 50 }, () => router.selectModel(profile, payload).provider);
    ...
    describe("routing bias in fallback", () => {
        it("fallback respects routingBias ordering", () => {
            ...
            const selections = Array.from({ length: 20 }, () => router.selectModel(profile, payload));
            const expected = selections[0];
            selections.forEach(sel => {
                expect(sel.name).toBe(expected.name);
            });
    ...
    describe("multivendor fallback", () => {
        it("anthropic → openai → google chain deterministic", () => {
            const profile = new AgentRoutingProfile(["claude-3.7"], ["gpt-4.1", "gemini-1.5"]);
            const payload = {
                model: "",
                messages: []
            };
            const results = Array.from({ length: 30 }, () => router.selectModel(profile, payload));
...
//# sourceMappingURL=c03-fallback-chain-determinism.test.js.map