...
for (const profile of PROFILES) {
    const selections = Array.from({ length: STRESS_COUNT }, () => router.selectModel(profile, payload));
    const serialized = selections.map(s => JSON.stringify({ name: s.name, provider: s.provider, bias: s.routingBias }));
...
(async () => {
    const agentResults = await Promise.all(Array.from({ length: STRESS_COUNT }, () => agent.runPlan("stress test")));
    ...
    // // console.log("\n📊 Test 3: Parallel Routing (100 concurrent)...");
    const parallelSelections = await Promise.all(Array.from({ length: STRESS_COUNT }, () => Promise.resolve(router.selectModel(PROFILES[0], payload))));
    const parallelSerialized = parallelSelections.map(s => JSON.stringify({ name: s.name, provider: s.provider }));
    ...
    const allPass = results.routing.identical === results.routing.runs &&
...
//# sourceMappingURL=stress-determinism.js.map