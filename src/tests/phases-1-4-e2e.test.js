...
describe("Phases 1-4 E2E Integration", () => {
    describe("Phase 1: Cloud Provider Dispatch", () => {
        ...
        it("enforces model validation before stub return", async () => {
            ...
            await expect(openrouterProvider.chat(invalidReq)).rejects.toThrow("Model not found");
        });
    });
    describe("Phase 2: Unified Grok Provider", () => {
        ...
        it("executes search via MCP client", async () => {
            ...
            expect(mcpClient.searchDocs).toHaveBeenCalledWith("How do I get started?", 10);
        });
    ...
    describe("Phase 3: RAG Fusion Pipeline", () => {
        ...
        beforeEach(() => {
                    ...
                    choices: [
                        {
                            message: {
                                role: "assistant",
                                content: "Based on the documentation, you should use API keys for authentication and rotate them regularly.",
                            },
                        },
                    ],
                }),
            ...
            grokProvider = new GrokProvider(mcpClient, modelClient);
            adapter = new GrokUnifiedAdapter({ name: "grok-rag", version: "1.0.0", timeout: 5000, retries: 2 }, grokProvider);
        });
        it("executes RAG query: search + build context + reasoning", async () => {
            ...
            expect(chatOutput.data.choices[0].message.content).toContain("authentication");
            // Verify context was passed to model
            expect(modelClient.chat).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("Cross-Phase Integration", () => {
        it("cloud provider can be swapped for Grok in adapter context", async () => {
            ...
            const adapter = new GrokUnifiedAdapter({ name: "test", version: "1.0", timeout: 5000, retries: 1 }, grok);
        ...
        it("all phases maintain deterministic behavior in test mode", () => {
            ...
            const input1 = { kind: "search", query: "test" };
            const input2 = { kind: "search", query: "test" };
            expect(JSON.stringify(input1)).toBe(JSON.stringify(input2));
        });
    });
});
//# sourceMappingURL=phases-1-4-e2e.test.js.map