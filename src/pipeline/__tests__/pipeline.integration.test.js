...
describe("Pipeline Integration - Harvester → PostProcessor → Auditor", () => {
            ...
            const segments = [
                {
                    id: "test-1",
                    source: "markdown",
                    content: "The approach primarily promoted by institutions—not by people themselves. Additionally, this enduring testament highlights its significance.",
                },
            ];
    ...
    describe("Pipeline with rewrite-labs profile", () => {
        test("applies Tier 1 + Tier 2 rules", async () => {
            ...
            const segment = {
                id: "test-tier-mix",
                source: "test",
                content: "In order to understand the pattern—this enduring testament shows significance. The approach is high-quality and serves as a model.",
            };
            const results = await pipeline.execute([segment]);
            expect(results[0].humanized).toBeDefined();
    ...
    describe("Pipeline with dry-run mode", () => {
        test("does not modify content in dry-run", async () => {
            ...
            stages.set("PostProcessor", PipelineFactory.wrapPostProcessor(postProcessor));
            const pipeline = PipelineFactory.createPipeline({ postProcessor: postProcessorConfig }, stages);
            const originalContent = "The approach primarily promoted—by institutions. Additionally, this enduring testament works.";
            const segment = {
                id: "dry-run-test",
                source: "test",
