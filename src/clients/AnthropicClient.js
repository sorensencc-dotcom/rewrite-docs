...
import { AnthropicResultSchema } from '../validation/schemas';
    ...
    async run(messages, options) {
        ...
        try {
            ...
            if (!parsed.success) {
                ...
                return makeError('ANTHROPIC_INVALID_RESPONSE', { reason: 'schema validation failed', errors: parsed.error }, adapter, startTime);
            }
            ...
            if (!parsed.data.text || parsed.data.text.trim().length === 0) {
                ...
                return makeError('ANTHROPIC_EMPTY_RESPONSE', { reason: 'API returned empty response' }, adapter, startTime);
            }
            ...
            if (!validateTextLength(text)) {
                ...
                return makeError('ANTHROPIC_OVERSIZE_OUTPUT', { reason: 'response exceeds size limits', length: text.length }, adapter, startTime);
            }
            const result = {
                text,
            ...
            return makeError('ANTHROPIC_API_ERROR', { reason: err instanceof Error ? err.message : 'unknown error' }, adapter, startTime);
        }
    }
}
