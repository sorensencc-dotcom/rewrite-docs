...
import { computeGRS, validateGRSInputs, computeAdaptiveRetryCap, computeRollbackSeverity, GRSInputs } from './grs';
...
describe('GRS Hybrid Formula', () => {
  ...
  describe('computeGRS', () => {
    it('should compute linear GRS when input < T=0.30', () => {
      const inputs: GRSInputs = { V: 0.1, R: 0.05, C: 0.08, I: 0.02 };
    ...
    it('should compute exponential GRS when input >= T=0.30', () => {
      const inputs: GRSInputs = { V: 0.3, R: 0.2, C: 0.25, I: 0.15 };
      const grs = computeGRS(inputs, config);
      ...
      const inputs2: GRSInputs = { V: 0.5, R: 0.4, C: 0.45, I: 0.35 };
    ...
    it('should return 0 for zero inputs', () => {
      const inputs: GRSInputs = { V: 0, R: 0, C: 0, I: 0 };
      const grs = computeGRS(inputs, config);
      expect(grs).toBe(0);
    });

    it('should return near 1 for maximum inputs', () => {
      const inputs: GRSInputs = { V: 1, R: 1, C: 1, I: 1 };
      const grs = computeGRS(inputs, config);
  ...
  describe('validateGRSInputs', () => {
    it('should accept valid inputs in [0, 1]', () => {
      const inputs: GRSInputs = { V: 0.5, R: 0.3, C: 0.7, I: 0.2 };
      expect(() => validateGRSInputs(inputs)).not.toThrow();
    });

    it('should reject inputs < 0', () => {
      const inputs: GRSInputs = { V: -0.1, R: 0.3, C: 0.7, I: 0.2 };
      expect(() => validateGRSInputs(inputs)).toThrow();
    });

    it('should reject inputs > 1', () => {
      const inputs: GRSInputs = { V: 1.5, R: 0.3, C: 0.7, I: 0.2 };
      expect(() => validateGRSInputs(inputs)).toThrow();
    });
  });
