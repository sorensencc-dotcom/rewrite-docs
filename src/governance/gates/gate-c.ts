 ...
 */
export async function gateCPromotionCheck(input: PromotionGateInput): Promise<GovernanceVerdictV2> {
  ...
  if (passed) {
    ...
    await pgQuery(
      `INSERT INTO governance_promotion_log (proposal_id, grs, ics, lcs, verdict, promoted_at)
       VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP)`,
      [input.proposalId, input.grs, input.impactScore, input.lineageConsistencyScore, "approved"]
    );

    // Write to canary_gate_results with grow decision
    await pgQuery(
      `INSERT INTO canary_gate_results (proposal_id, growth_decision, created_at)
       VALUES ($1, $2, CURRENT_TIMESTAMP)`,
      [input.proposalId, "grow"]
    );
  } else {
    // Log rejection
    await pgQuery(
      `INSERT INTO governance_promotion_log (proposal_id, grs, ics, lcs, verdict, promoted_at)
       VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP)`,
      [input.proposalId, input.grs, input.impactScore, input.lineageConsistencyScore, "rejected"]
    );
  }
