...
export class GovernanceConfigLoader {
    static load() {
        ...
        const hybridThreshold = this.parseFloatWithValidation(process.env.HYBRID_THRESHOLD || "0.30", 0.30, 0.20, 0.40);
        ...
        // Promotion caps
        const promotionCaps = {
            maxGRS: this.parseFloatWithValidation(process.env.PROMOTION_CAP_MAX_GRS || "0.70", 0.70, 0.0, 1.0),
            maxImpactScore: this.parseFloatWithValidation(process.env.PROMOTION_CAP_MAX_IMPACT || "0.60", 0.60, 0.0, 1.0),
    ...
    static parseFloatWithValidation(value, defaultVal, minInclusive, maxInclusive) {
        const parsed = parseFloat(value);
        if (isNaN(parsed))
            return defaultVal;
        if (parsed < minInclusive || parsed > maxInclusive) {
            throw new Error(`GovernanceConfig value out of range: ${parsed} not in [${minInclusive}, ${maxInclusive}]`);
        }
        return parsed;
    }
}
