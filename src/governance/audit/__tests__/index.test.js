...
describe("SkillAudit", () => {
    ...
    describe("audit", () => {
        ...
        it("should return FAIL for skill with eval", async () => {
            ...
            expect(result.policies_triggered.some((p) => p.id === "PROMPT_INJECTION_PATTERNS")).toBe(true);
        });
        it("should return WARN for skill with flags", async () => {
            const skill = createTestSkill("flagged", JSON.stringify({ name: "Test" }) // Missing required sections
            );
            const context = createTestContext();
            const result = await audit.audit(skill, context);
        ...
        it("should set correct verdict based on severity", async () => {
            const skill = createTestSkill("hardcoded-key", 'const apiKey = "EXAMPLE_FAKE_API_KEY_DO_NOT_USE";');
            const context = createTestContext();
            const result = await audit.audit(skill, context);
            expect(result.verdict).toBe("FAIL");
    ...
    describe("risk scoring", () => {
        it("should calculate correct risk score", async () => {
            const skill = createTestSkill("unsafe", "eval(x); password = 'EXAMPLE_FAKE_PASSWORD_DO_NOT_USE';");
            const context = createTestContext();
            const result = await audit.audit(skill, context);
