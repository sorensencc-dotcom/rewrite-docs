...
describe("DeterministicAudit", () => {
    ...
    describe("check", () => {
        it("should detect eval() pattern", () => {
            ...
            expect(hard_fails.some((p) => p.id === "PROMPT_INJECTION_PATTERNS")).toBe(true);
        });
        it("should detect hardcoded API keys", () => {
            const content = 'const apiKey = "EXAMPLE_FAKE_API_KEY_DO_NOT_USE";';
            const { hard_fails } = audit.check(content, SkillPolicies);
      ...
      `;
        ...
        it("should detect hardcoded passwords", () => {
            const content = 'const password = "EXAMPLE_FAKE_PASSWORD_DO_NOT_USE";';
            const { hard_fails } = audit.check(content, SkillPolicies);
            expect(hard_fails.some((p) => p.id === "CREDENTIAL_EXPOSURE")).toBe(true);
        });
