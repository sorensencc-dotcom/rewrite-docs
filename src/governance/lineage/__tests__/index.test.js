import { SkillLineage } from "../index";
class MockDatabase {
    tables = new Map();
    ...
    async query(sql, params) {
        ...
        if (sql.includes("SELECT * FROM skill_lineage WHERE skill_id = ?")) {
            ...
            if (sql.includes("ORDER BY audit_timestamp")) {
                results.sort((a, b) => {
                    const aTime = new Date(a.audit_timestamp).getTime();
                    const bTime = new Date(b.audit_timestamp).getTime();
                    return aTime - bTime;
                });
            }
            return results;
        ...
        if (sql.includes("SELECT DISTINCT skill_id FROM skill_lineage")) {
            const lineage = this.tables.get("skill_lineage") || [];
            const policy = params?.[0]?.replace(/%/g, ""); // Strip SQL LIKE wildcards
            return lineage.filter((row) => row.policies_triggered.includes(policy));
        }
        return [];
    }
    clear() {
...
function createTestAuditResult(skillId, verdict) {
    return {
        skill_id: skillId,
...
describe("SkillLineage", () => {
    ...
    describe("record", () => {
        ...
        it("should serialize policies triggered as JSON", async () => {
            ...
            const parsed = JSON.parse(records[0].policies_triggered);
