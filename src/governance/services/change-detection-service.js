...
const log = (skillId, msg, data) => console.log(`[SCP-DETECT:${skillId}] ${msg}`, data || "");
const logError = (skillId, msg, error) => console.error(`[SCP-DETECT:${skillId}] ERROR: ${msg}`, error);
export class ChangeDetectionService {
    ...
    async detectChanges(skillId) {
        ...
        try {
            ...
            if (!skill) {
                return this.buildErrorResult(skillId, "unknown", "not-found", "Skill not registered in manifest");
            }
            ...
            if (!fs.existsSync(skill.local_path)) {
                ...
                return this.buildErrorResult(skillId, skill.skill_name, "not-found", `Local file not found: ${skill.local_path}`);
            }
            // Read local content
            ...
            const remoteContent = await this.fetchRemoteWithRetry(skill.source_repo_url, skill.source_repo_branch, skill.source_repo_path, skillId);
            if (!remoteContent) {
            ...
            log(skillId, `Detection complete (${changedLines} lines changed) in ${Date.now() - startMs}ms`);
            return result;
        }
        catch (error) {
            ...
            return this.buildErrorResult(skillId, "unknown", "error", error instanceof Error ? error.message : "Unknown error");
        }
    }
    // Fetch remote file with exponential backoff retry
    async fetchRemoteWithRetry(repoUrl, branch, remotePath, skillId) {
        let attempt = 0;
    ...
    async fetchRemoteFile(repoUrl, branch, remotePath) {
        const rawUrl = this.buildGithubRawUrl(repoUrl, branch, remotePath);
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error(`Timeout fetching ${rawUrl}`)), this.timeoutMs);
            ...
            try {
                ...
                https.get(rawUrl, { timeout: this.timeoutMs }, (res) => {
    ...
    buildGithubRawUrl(repoUrl, branch, remotePath) {
    ...
    generateUnifiedDiff(local, remote) {
    ...
    async recordDetectionEvent(skillId, event) {
        try {
            const query = `
      ...
      `;
            ...
            log(skillId, `Recorded detection event: ${event.status} (${event.reason})`);
        }
        catch (error) {
    ...
    // Build error result
    buildErrorResult(skillId, skillName, status, errorMessage) {
        return {
            skillId,
            skillName,
            hasChanges: false,
    ...
    async queryModifiedSkills() {
        const query = "SELECT * FROM skill_manifest WHERE is_locally_modified = 1 ORDER BY updated_at DESC LIMIT 100";
        return this.db.query(query);
    }
    // Clear modification flag after PR submission
    async clearModificationFlag(skillId) {
        const query = `
      UPDATE skill_manifest
