...
describe("ContributionAgent", () => {
    ...
    describe("createPullRequest - Happy Path", () => {
        it("should create PR from detected changes", async () => {
            ...
            jest.spyOn(agent, "getDefaultBranchShaWithRetry").mockResolvedValueOnce("abc123def456");
            jest.spyOn(agent, "createBranchWithRetry").mockResolvedValueOnce("abc123def456");
            jest.spyOn(agent, "commitFileWithRetry").mockResolvedValueOnce("commit789");
            jest.spyOn(agent, "createPRWithRetry").mockResolvedValueOnce({
                number: 42,
    ...
    describe("createPullRequest - Validation", () => {
        it("should reject if no changes detected", async () => {
            ...
            await expect(agent.createPullRequest(noChangeRequest)).rejects.toThrow("No changes detected");
        });
        it("should reject on invalid GitHub URL", async () => {
            ...
            await expect(agent.createPullRequest(badUrlRequest)).rejects.toThrow("Invalid GitHub URL");
        });
    });
    describe("createPullRequest - GitHub API Errors", () => {
        it("should handle 401 authentication failure", async () => {
            ...
            jest.spyOn(agent, "getDefaultBranchShaWithRetry").mockRejectedValueOnce(new Error("401 Bad credentials"));
            await expect(agent.createPullRequest(mockRequest)).rejects.toThrow("Bad credentials");
        });
        it("should handle 404 repository not found", async () => {
            jest.spyOn(require("fs"), "readFile").mockImplementation((path, enc, cb) => {
                cb(null, "Content");
            });
            jest.spyOn(agent, "getDefaultBranchShaWithRetry").mockRejectedValueOnce(new Error("404 Repository not found"));
            await expect(agent.createPullRequest(mockRequest)).rejects.toThrow("not found");
        });
        it("should handle 422 branch already exists (retry idempotency)", async () => {
            jest.spyOn(require("fs"), "readFile").mockImplementation((path, enc, cb) => {
                cb(null, "Content");
            });
            jest.spyOn(agent, "getDefaultBranchShaWithRetry").mockResolvedValueOnce("abc123");
            // First attempt: 422 branch exists; agent returns baseSha (idempotent)
            jest.spyOn(agent, "createBranchWithRetry").mockResolvedValueOnce("abc123");
            jest.spyOn(agent, "commitFileWithRetry").mockResolvedValueOnce("commit789");
            jest.spyOn(agent, "createPRWithRetry").mockResolvedValueOnce({
                number: 43,
    ...
    describe("createPullRequest - Rate Limiting", () => {
        it("should handle 429 rate limit", async () => {
            ...
            rateLimitError.retryAfter = 3600000;
            jest.spyOn(agent, "getDefaultBranchShaWithRetry").mockRejectedValueOnce(rateLimitError);
            await expect(agent.createPullRequest(mockRequest)).rejects.toThrow("rate limit");
        });
    });
    describe("createPullRequest - Network Failures", () => {
        it("should retry on timeout and fail after max attempts", async () => {
            ...
            jest.spyOn(agent, "getDefaultBranchShaWithRetry").mockRejectedValueOnce(new Error("GitHub API timeout"));
            await expect(agent.createPullRequest(mockRequest)).rejects.toThrow("timeout");
        });
        it("should handle file read errors", async () => {
            jest.spyOn(require("fs"), "readFile").mockImplementation((path, enc, cb) => {
                cb(new Error("ENOENT: no such file"));
            });
            await expect(agent.createPullRequest(mockRequest)).rejects.toThrow("ENOENT");
        });
    });
    describe("parseGitHubUrl", () => {
        it("should extract owner and repo from HTTPS URL", () => {
            const metadata = agent.parseGitHubUrl("https://github.com/anthropics/claude-skills", "test", mockRequest.diffSummary);
        ...
        it("should extract owner and repo from SSH URL", () => {
            const metadata = agent.parseGitHubUrl("git@github.com:anthropics/claude-skills.git", "test", mockRequest.diffSummary);
            expect(metadata.owner).toBe("anthropics");
            expect(metadata.repo).toBe("claude-skills");
        });
        it("should reject invalid URLs", () => {
            expect(() => {
                agent.parseGitHubUrl("https://example.com/repo", "test", mockRequest.diffSummary);
...
//# sourceMappingURL=contribution-agent.test.js.map