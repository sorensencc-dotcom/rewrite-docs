...
describe("ManifestService", () => {
    ...
    describe("updateContributionStatus", () => {
        it("should update PR status to merged", async () => {
            ...
            await service.updateContributionStatus("test-skill", 42, "merged", "Merged successfully");
            expect(mockDb.execute).toHaveBeenCalled();
    ...
    describe("Error Handling - registerSkill", () => {
        it("should propagate database constraint violations", async () => {
            ...
            await expect(service.registerSkill(skill)).rejects.toThrow("Duplicate entry");
        });
    });
    describe("Error Handling - getSkillById", () => {
        ...
        it("should propagate database query errors", async () => {
            ...
            await expect(service.getSkillById("test-skill")).rejects.toThrow("Connection timeout");
        });
    });
    describe("Error Handling - recordContribution", () => {
        it("should propagate foreign key constraint errors", async () => {
            ...
            mockDb.execute.mockRejectedValueOnce(new Error("Foreign key constraint failed"));
            await expect(service.recordContribution(contrib)).rejects.toThrow("Foreign key");
        });
    });
    describe("Error Handling - updateContributionStatus", () => {
        it("should handle database errors during status update", async () => {
            ...
            await expect(service.updateContributionStatus("test", 42, "merged", "Note")).rejects.toThrow("DB locked");
        });
    });
    describe("Error Handling - markUnavailable", () => {
        it("should propagate database errors", async () => {
            mockDb.execute.mockRejectedValueOnce(new Error("Connection lost"));
            await expect(service.markUnavailable("test", "404")).rejects.toThrow("Connection lost");
        });
    });
});
//# sourceMappingURL=manifest-service.test.js.map