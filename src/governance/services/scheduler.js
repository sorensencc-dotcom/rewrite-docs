...
const log = (msg) => console.log(`[SCP-SCHEDULER] ${msg}`);
const logError = (msg, error) => console.error(`[SCP-SCHEDULER] ERROR: ${msg}`, error);
export class Scheduler {
    ...
    scheduleDailyRun() {
        ...
        const now = new Date();
        const nextRun = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), hours, minutes));
    ...
    scheduleWeeklyReport() {
        ...
        const delayMs = nextRun.getTime() - now.getTime();
        log(`Weekly report scheduled for ${nextRun.toISOString()} (in ${Math.floor(delayMs / 1000)}s)`);
        const timer = setTimeout(() => {
    ...
    schedulePRStatusPolling() {
        ...
        const delayMs = nextRun.getTime() - now.getTime();
        log(`PR status polling scheduled for ${nextRun.toISOString()} (in ${Math.floor(delayMs / 1000)}s)`);
        const timer = setTimeout(() => {
    ...
    scheduleCleanup() {
        const now = new Date();
        const nextRun = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 3, 0));
        if (nextRun.getTime() <= now.getTime()) {
            nextRun.setUTCDate(nextRun.getUTCDate() + 1);
        }
        const delayMs = nextRun.getTime() - now.getTime();
    ...
    async runDailyChangeDetection() {
        log("Running daily change detection...");
        try {
            // Get all registered skills
            const skills = await this.db.query("SELECT skill_id, skill_name, source_repo_url, source_repo_branch, local_path FROM skill_manifest WHERE is_available = 1");
            if (skills.length === 0) {
            ...
            for (const skill of skills) {
                try {
                    ...
                    try {
                        ...
                        // Notify submission
                        await this.notifier.notifySubmitted(skill.skill_id, skill.skill_name, prResult.prNumber, prResult.prUrl, result.summary.linesAdded, result.summary.linesDeleted);
                    }
                    catch (prError) {
            ...
            log(`Daily run complete: ${modified} modified, ${contributed} PRs created (out of ${skills.length} skills)`);
        }
        catch (error) {
    ...
    async runWeeklyReport() {
        ...
        `, [sevenDaysAgo.toISOString()]);
            ...
            log(`Weekly report: ${record.total_prs || 0} PRs (${record.merged || 0} merged, ${record.open || 0} open, ${record.closed || 0} closed)`);
    ...
    async runPRStatusPolling() {
      ...
      `);
            ...
            for (const pr of openPRs) {
                try {
                    const snapshot = await this.tracker.checkAndUpdatePRStatus(pr.skill_id, pr.pr_number, pr.upstream_repo_url, pr.skill_name);
                    if (snapshot.status !== "open") {
                        ...
                        try {
                            if (snapshot.status === "merged") {
                                await this.notifier.notifyMerged(pr.skill_id, pr.skill_name, pr.pr_number, pr.upstream_repo_url, // Will need PR URL from DB
                                new Date().toISOString());
                            }
                            else if (snapshot.status === "closed") {
                                await this.notifier.notifyClosed(pr.skill_id, pr.skill_name, pr.pr_number, pr.upstream_repo_url);
                            }
                        }
                        catch (notifyError) {
                    ...
                    await this.db.execute("UPDATE skill_contributions SET last_checked_at = CURRENT_TIMESTAMP WHERE skill_id = ? AND pr_number = ?", [pr.skill_id, pr.pr_number]);
                }
                catch (error) {
