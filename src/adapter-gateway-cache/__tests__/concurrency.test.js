...
describe("Concurrency Tests", () => {
    ...
    it("should handle 100 parallel requests with same input", async () => {
        ...
        expect(responses.every((r) => r.success && r.data.result === 10)).toBe(true);
    ...
    it("should serialize concurrent writes to same key", async () => {
        const promises = Array(10)
            .fill(null)
            .map((_, i) => gateway.invoke("test-adapter", { value: i }).then(() => ({ index: i })));
    ...
    it("should handle stress with 1000 mixed operations", async () => {
        ...
        for (let i = 0; i < 1000; i++) {
            ...
            if (op < 0.7) {
                operations.push(gateway.invoke("test-adapter", { value: Math.floor(Math.random() * 10) }));
            }
            else if (op < 0.9) {
                operations.push(gateway.invoke("test-adapter", { value: Math.floor(Math.random() * 10) }, true));
            }
            else {
