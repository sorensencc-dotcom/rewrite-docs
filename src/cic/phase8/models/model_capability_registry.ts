...
import { ModelDescriptor, estimateCost } from '../types/model_descriptor.js';
...
export class ModelCapabilityRegistry {
  ...
  ): Array<{ model: ModelDescriptor; estimatedCostUsd: number }> {
  ...
  getFallbackModel(): ModelDescriptor | undefined {
    ...
    const candidates = Array.from(this.models.values()).filter(
      m => (m.tier === 'standard' || m.tier === 'economy') && m.enabled
    );
