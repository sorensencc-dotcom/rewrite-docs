...
import { CICIntegrationAdapterPhase8, Phase8Config } from '../../phase8/integration/cic_integration_adapter_phase8.js';
import { ModelDescriptor } from '../../phase8/types/model_descriptor.js';
...
const defaultConfig: Phase8Config = {
  telemetrySink: process.env.COST_SINK as any || 'prometheus',
  costPolicyConfig: {
...
export class Phase8Adapter {
  private static instance: CICIntegrationAdapterPhase8 | null = null;

  private static getInstance(): CICIntegrationAdapterPhase8 {
    if (!this.instance) {
   ...
   */
  static async getCostOptimizationSignals(service: string): Promise<CostOptimizationSlice> {
    try {
