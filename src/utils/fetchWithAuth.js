...
async function getRealToken(tokenUrl) {
    ...
    if (!resp.ok)
        throw new Error(`OIDC token error: ${resp.status}`);
    const json = await resp.json();
    return json.access_token;
}
export async function fetchWithAuth(path, payload, options = {}) {
    if (options.mock) {
...
function simulateMockResponse(path, payload) {
