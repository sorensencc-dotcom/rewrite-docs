...
export async function executeS3(code, options) {
    ...
    const vmConfigHash = await runtime.boot(options.seed);
    ...
    if (options.createSnapshot) {
        const snap = await snapshotMgr.createSnapshot(vmId, `/tmp/mem_${vmId}.snap`, `/tmp/vm_${vmId}.snap`);
...
//# sourceMappingURL=s3-exec-firecracker-v3.js.map