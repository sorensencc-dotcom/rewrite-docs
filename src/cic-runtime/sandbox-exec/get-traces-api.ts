...
import { TracesResponse } from './api-types';
import { pgQuery } from '../audit-log/postgres-client';

export async function getTracesApi(req: Request, res: Response) {
  const { runId } = req.params;

  try {
    const result = await pgQuery(
      'SELECT network_trace_json, file_access_json FROM cic_audit_log WHERE run_id = $1',
      [runId]
    );
