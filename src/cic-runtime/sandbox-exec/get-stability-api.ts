...
import { StabilityResponse } from './api-types';
import { pgQuery } from '../audit-log/postgres-client';

export async function getStabilityApi(req: Request, res: Response) {
  ...
  try {
    ...
    if (avgScore > 0.30) level = 'high';
    else if (avgScore > 0.20) level = 'medium';
