...
export class SnapshotManager {
  ...
  async createSnapshot(vmId: string, memoryFilePath: string, snapshotFilePath: string) {
  ...
  async restoreSnapshot(vmId: string, snapshotHash: string) {
  ...
  private async hashFileStream(filePath: string): Promise<string> {
    if (!fs.existsSync(filePath)) return "00000000";
