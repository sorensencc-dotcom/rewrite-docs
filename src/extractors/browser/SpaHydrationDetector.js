...
export class SpaHydrationDetector {
    ...
    mutationWindowMs = 300;
    stabilityThresholdMs = 100;
    stabilityNodeDeltaPercent = 5;
    async detect(page) {
    ...
    async observeHydration(page, initialNodeCount) {
    ...
    classifyFramework(signals) {
        if (signals.reactNextMarkers)
            return "react";
        if (signals.webflowMarker)
            return "webflow";
        if (signals.framerMarker)
            return "framer";
        if (signals.wixMarker)
            return "wix";
        return "unknown";
    }
...
//# sourceMappingURL=SpaHydrationDetector.js.map