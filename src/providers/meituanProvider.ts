// src/providers/meituanProvider.ts
...
export const meituanProvider: Provider = {
  name: "meituan",
  ...
  chat: async (req: UnifiedChatRequest): Promise<CloudChatResponse> => {
    ...
    if (
      (process.env.NODE_ENV === "test" || process.env.MOCK_PROVIDERS === "1") &&
      !apiKey
    ) {
      return {
    ...
    try {
      const response = await fetchWithTimeout(
        "https://api.meituan.com/v1/chat/completions",
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          `Meituan API error: ${response.status} ${response.statusText}`
        );
      }
