...
export const cloudProviders: Record<string, Provider> = {
...
export function getCloudProvider(providerName: string): Provider {
  const provider = cloudProviders[providerName];
  if (!provider) throw new Error(`Unknown cloud provider: ${providerName}`);
  return provider;
}

export function listCloudModels(enabledProviders?: string[]): string[] {
