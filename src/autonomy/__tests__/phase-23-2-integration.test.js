...
describe.skip('Phase 23.2: MemoryStore Integration', () => {
    ...
    test('Query API filters by event type correctly', async () => {
        ...
        expect(results.some((e) => e.id === appended.id)).toBe(true);
    });
    /**
    ...
    test.skip('End-to-end: Signal detection → MemoryStore → query pipeline', async () => {
        ...
        expect(retrieved.payload.metadata.confidence).toBe(0.99);
        expect(retrieved.payload.metadata.affectedPhases).toContain('Phase 24');
    });
});
//# sourceMappingURL=phase-23-2-integration.test.js.map