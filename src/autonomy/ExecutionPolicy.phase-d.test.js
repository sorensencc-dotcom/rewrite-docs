 ...
 */
import { ExecutionMode, ExecutionPolicyEngine, } from './ExecutionPolicy';
import { TaskMetadataStore, } from './TaskMetadataStore';
import { ExecutionPolicyInterceptor } from './ExecutionPolicyInterceptor';
describe('Phase D: Comprehensive Execution Policy Testing', () => {
    ...
    describe('E2E Docker Integration', () => {
        ...
        it('Docker build with unauthorized tool fails fast in UNATTENDED mode', () => {
            ...
            store.recordToolCall('docker-build-unauthorized', unauthorizedTool, false, 'denied', 'Not in preapproved set');
            // With exitOnUnauthorized=true, task should fail
            const completed = store.completeExecution('docker-build-unauthorized', 'FAILURE', 'Unauthorized tool call', unauthorizedTool);
    ...
    describe('Failure Cases & Edge Behavior', () => {
        it('UNATTENDED mode with exitOnUnauthorized=false continues after denial', () => {
            ...
            // Denied tool
            store.recordToolCall('fail-but-continue', 'Bash(curl evil)', false, 'denied', 'Not preapproved');
    ...
    describe('Audit Trail Validation', () => {
        it('exports detailed audit log with all metadata', () => {
            ...
            store.recordToolCall('audit-detailed', 'Bash(unauthorized)', false, 'denied', 'Not in preapproved set');
        ...
        it('clears old execution records after retention period', () => {
            ...
            if (record0)
                record0.endedAt = new Date(Date.now() - 2 * 60 * 60 * 1000); // 2 hours ago
            if (record1)
                record1.endedAt = new Date(Date.now() - 30 * 60 * 1000); // 30 min ago (recent)
        ...
        it('exports all execution histories for audit compliance', () => {
            ...
            expect(records.map((r) => r.taskId)).toEqual(expect.arrayContaining(taskIds));
        });
    });
...
//# sourceMappingURL=ExecutionPolicy.phase-d.test.js.map