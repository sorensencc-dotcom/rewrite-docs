 ...
 */
export function useConsolePolling(intervals = {}) {
    const { health: healthData, fetch: fetchHealth, } = useHealthStatus();
    const { pipelines, fetch: fetchPipelines, } = usePipelines();
    const { alerts, fetch: fetchAlerts, } = useAlerts();
    const healthIntervalRef = useRef();
    const pipelinesIntervalRef = useRef();
    const alertsIntervalRef = useRef();
    const start = useCallback(() => {
        ...
        return () => {
            ...
            if (pipelinesIntervalRef.current)
                clearInterval(pipelinesIntervalRef.current);
