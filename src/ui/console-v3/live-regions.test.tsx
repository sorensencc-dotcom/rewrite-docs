...
describe('Live Regions (Phase 3.6 Stream C)', () => {
  describe('StatusLive', () => {
    it('renders with role="status" and aria-live="polite"', () => {
      const { container } = render(<StatusLive message="Test status" />);
    ...
    it('displays status message without focus change', () => {
      const { rerender } = render(<StatusLive message="" />);
      const region = screen.getByTestId('status-live');

      const initialFocused = document.activeElement;

      rerender(<StatusLive message="Pipeline started successfully" />);
    ...
    it('updates message and clears after duration', async () => {
      const { rerender } = render(<StatusLive message="Status 1" />);
      const region = screen.getByTestId('status-live');

      expect(region.textContent).toContain('Status 1');

      rerender(<StatusLive message="Status 2" />);
    ...
    it('has aria-label for screen reader context', () => {
      render(<StatusLive label="Custom status label" />);
  ...
  describe('AlertLive', () => {
    it('renders with role="alert" and aria-live="assertive"', () => {
      const { container } = render(<AlertLive message="Critical alert" />);
    ...
    it('displays alert message without focus change', () => {
      const { rerender } = render(<AlertLive message="" />);
      const region = screen.getByTestId('alert-live');

      const initialFocused = document.activeElement;

      rerender(<AlertLive message="Database connection failed" />);
    ...
    it('persists alert message (no auto-clear)', async () => {
      const { rerender } = render(<AlertLive message="Critical: Service unavailable" />);
      const region = screen.getByTestId('alert-live');
    ...
    it('has aria-label for screen reader context', () => {
      render(<AlertLive label="System alerts" />);
  ...
  describe('LogLive', () => {
    it('renders with role="status" and aria-live="polite"', () => {
      const { container } = render(<LogLive message="Operation started" />);
    ...
    it('displays latest log message', () => {
      const { rerender } = render(<LogLive />);
      const region = screen.getByTestId('log-live');

      rerender(<LogLive message="Task 1 complete" />);
      expect(region.textContent).toContain('Task 1 complete');

      rerender(<LogLive message="Task 2 complete" />);
    ...
    it('maintains log history (last 5 messages)', () => {
      ...
      for (let i = 1; i <= 6; i++) {
        rerender(<LogLive message={`Log message ${i}`} />);
      }
    ...
    it('has aria-label for screen reader context', () => {
      render(<LogLive label="Operation progress" />);
  ...
  describe('ConsoleLiveRegions', () => {
    ...
    it('is visually hidden (display: none)', () => {
      const { container } = render(<ConsoleLiveRegions />);
    ...
    it('container is accessible via test ID', () => {
      render(<ConsoleLiveRegions data-testid="my-live-regions" />);
  ...
  describe('PollingAnnouncements.formatHealthAnnouncement', () => {
    it('returns null when status unchanged', () => {
      const current = { status: 'OK' as const, serviceCount: 5, timestamp: Date.now() };
      const previous = { status: 'OK' as const, serviceCount: 5, timestamp: Date.now() - 1000 };
    ...
    it('announces health recovered', () => {
      const current = { status: 'OK' as const, serviceCount: 5, timestamp: Date.now() };
      const previous = { status: 'DOWN' as const, serviceCount: 0, timestamp: Date.now() - 1000 };
    ...
    it('announces health degraded', () => {
      const current = { status: 'DEGRADED' as const, serviceCount: 3, timestamp: Date.now() };
      const previous = { status: 'OK' as const, serviceCount: 5, timestamp: Date.now() - 1000 };
    ...
    it('announces system down as critical alert', () => {
      const current = { status: 'DOWN' as const, serviceCount: 0, timestamp: Date.now() };
      const previous = { status: 'OK' as const, serviceCount: 5, timestamp: Date.now() - 1000 };
  ...
  describe('PollingAnnouncements.formatPipelineAnnouncement', () => {
    it('returns empty array when no state changes', () => {
      const pipelines = [
        { id: 'p1', name: 'Pipeline 1', state: 'idle' as const },
        { id: 'p2', name: 'Pipeline 2', state: 'running' as const, currentTask: 'Processing' },
      ];
    ...
    it('announces pipeline started', () => {
      const pipelines = [
        { id: 'p1', name: 'Pipeline 1', state: 'running' as const, currentTask: 'Init' },
      ];
    ...
    it('announces pipeline paused', () => {
      const pipelines = [
        { id: 'p1', name: 'Pipeline 1', state: 'paused' as const },
      ];
    ...
    it('announces pipeline failed as alert', () => {
      const pipelines = [
        { id: 'p1', name: 'Pipeline 1', state: 'failed' as const },
      ];
      const previousStates = new Map([['p1', 'running']]);
  ...
  describe('PollingAnnouncements.formatAlertAnnouncement', () => {
    it('returns null when no new critical alerts', () => {
      const alerts = [
        { id: 'a1', severity: 'info' as const, service: 'Service 1', message: 'Info', duration: 0 },
      ];
