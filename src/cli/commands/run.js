...
export function createRunCommand() {
    const command = new Command()
        ...
        .option("--humanize", "Enable Humanizer PostProcessor stage (AI fingerprint removal)", false)
        .option("--humanize-profile <profile>", "Humanizer profile (default|rewrite-labs|custom)", "default")
        .option("--humanize-tiers <tiers>", "Rule tiers for custom profile (comma-separated: 1,2,3)", "")
        .option("--diff", "Show before/after diffs for humanized segments (implies --humanize)", false)
        .option("--dry-run", "Run pipeline without modifying data", false)
        .action(async (options) => {
        try {
            // Parse options
            const humanizeEnabled = options.humanize || options.diff;
            const profile = options.humanizeProfile || "default";
            const isDiff = options.diff;
            const isDryRun = options.dryRun;
            // Parse custom tiers if specified
            const ruleTiers = {};
            if (profile === "custom" && options.humanizeTiers) {
                const tiers = options.humanizeTiers.split(",");
                if (tiers.includes("1"))
                    ruleTiers["tier1"] = true;
                if (tiers.includes("2"))
                    ruleTiers["tier2"] = true;
                if (tiers.includes("3"))
                    ruleTiers["tier3"] = true;
            }
            ...
            const sampleSegments = [
                {
                    id: "sample-1",
                    source: "test",
                    content: "The term is primarily promoted by Dutch institutions—not by people themselves. This enduring testament to the broader trend highlights its significance.",
                },
                {
                    id: "sample-2",
                    source: "test",
                    content: "Additionally, this groundbreaking approach serves as a pivotal moment in the evolution of modern systems, underscoring the importance of strategic alignment.",
                },
            ];
            // Execute pipeline
