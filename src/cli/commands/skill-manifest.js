...
export function createManifestCommand(manifestService, options) {
    ...
    cmd
        ...
        try {
            ...
            if (!/^[a-z0-9\-_]+$/.test(skillId)) {
                throw new Error("Invalid skill-id. Use lowercase letters, numbers, dash, underscore only.");
            }
            ...
            try {
                ...
                throw new Error(`Invalid GitHub URL: ${repoUrl}. Expected: https://github.com/owner/repo`);
            }
            ...
            if (!resolvedPath.startsWith(resolvedSkillsDir)) {
                throw new Error(`Path traversal detected. Local path must be under ${skillsDir}`);
            }
        ...
        cmd
            ...
            try {
                ...
                if (result.summary.status === "not-found") {
                    throw new Error(`Skill not found in manifest. Register with: /skill-manifest register <url> ${skillId}`);
                }
                if (result.summary.status === "network-fail") {
                    console.warn("⚠️  Could not reach upstream after retries. Showing cached state.");
                    console.warn(`   Error: ${result.summary.errorMessage}`);
                }
                if (result.summary.status === "error") {
                    throw new Error(`Detection error: ${result.summary.errorMessage}`);
                }
                // Display results
                if (result.summary.status === "no-change") {
                    ...
                    console.log(`📝 ${result.skillName}`);
                    console.log(`   ${result.summary.linesAdded} added, ${result.summary.linesDeleted} deleted, ${result.summary.linesModified} modified`);
    ...
    if (options?.db) {
        ...
        cmd
            ...
            try {
                ...
                const skill = await manifestService.getSkillById(skillId);
                if (!skill) {
                    throw new Error(`Skill not found: ${skillId}. Register with: /skill-manifest register <url> ${skillId}`);
                }
                ...
                console.log(`   ✓ Changes found: ${diffResult.summary.linesAdded} added, ${diffResult.summary.linesDeleted} deleted`);
                // Create PR
    ...
    if (options?.db) {
        ...
        cmd
            ...
            try {
                ...
                console.log("   PR #  | Status     | Review              | Comments | Commit");
                console.log("   ------|------------|---------------------|----------|--------");
                for (const status of statuses) {
                    ...
                    console.log(`   ${prStr} | ${statusStr} | ${reviewStr} | ${commentsStr} | ${commitStr}`);
                }
                console.log("\n");
                // Summary stats
                ...
                const approved = statuses.filter((s) => s.reviewState === "approved").length;
                const changesRequested = statuses.filter((s) => s.reviewState === "changes-requested").length;
