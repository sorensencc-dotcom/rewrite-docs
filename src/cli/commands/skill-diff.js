...
export function createDiffCommand(db) {
    ...
    const command = new Command("diff")
        ...
        try {
            ...
            if (!isValidSkillId(skillId)) {
                console.error("❌ Invalid skill-id. Use lowercase letters, numbers, dash, underscore only.");
                process.exit(1);
            }
            ...
            if (result.summary.status === "not-found") {
                console.error(`❌ Skill not found in manifest. Register with: /skill-manifest register <url> ${skillId}`);
                process.exit(1);
            }
...
function displayDiffResult(result, options) {
    ...
    if (summary.status === "modified") {
        console.log(`📝 ${skillName}`);
        console.log(`   ${summary.linesAdded} added, ${summary.linesDeleted} deleted, ${summary.linesModified} modified`);
    ...
    console.log(`⚠️  Detection status: ${summary.status} — ${summary.errorMessage}`);
}
