...
export class AuditAgent extends BaseAgent {
    ...
    async auditImplementation(result, secondaryOverride) {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`Audit primary models failed: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
    ...
    buildAuditPrompt(result) {
        return [
            {
                role: "system",
                content: "You are an audit agent responsible for verifying correctness and consistency. Provide detailed reasoning."
            },
            { role: "user", content: `Audit this result for correctness and coherence:\n\n${result}` }
        ];
    }
    computeConsistency(primaryText, secondaryText) {
        const issues = [];
        let score = 1.0;
        ...
        const hasHallucMarkers = hallucMarkers.some((marker) => primaryText.toLowerCase().includes(marker)) ||
        ...
        const lengthRatio = Math.max(primaryText.length / (secondaryText.length + 1), secondaryText.length / (primaryText.length + 1));
        if (lengthRatio > 2.5) {
            score -= 0.2;
...
//# sourceMappingURL=auditAgent.js.map