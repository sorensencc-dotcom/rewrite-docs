 ...
 */
function mapIsolationLevelToRank(level) {
    switch (level) {
        case "container":
 ...
 * - cost preference
 */
export function selectSandboxTier(req) {
    const entries = Object.entries(sandboxConfig.sandboxMatrix);
    // 1. Filter by trust + sensitivity + taskType
    const filtered = entries.filter(([tierId, cfg]) => {
        return (cfg.defaultFor.trustLevels.includes(req.trustLevel) &&
 ...
 */
function pickByCostAndLatency(candidates, costBudget, slo) {
 ...
 */
export function routeWithSandbox(req) {
...
//# sourceMappingURL=route-with-sandbox.js.map