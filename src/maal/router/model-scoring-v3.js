export class ModelScoringV3 {
     ...
     */
    static computeTrustScore(driftScore, sloViolationRate, // 0.0 to 1.0
    reproScore // 0.0 to 1.0
    ) {
        let score = 100;
        // Penalize for drift
        if (driftScore > 0.3)
        ...
        if (sloViolationRate > 0.05)
            score -= 30;
        else if (sloViolationRate > 0.01)
            score -= 10;
        // Penalize for non-determinism
        if (reproScore < 0.95)
            score -= (1.0 - reproScore) * 100;
        return Math.max(0, Math.floor(score));
    }
}
//# sourceMappingURL=model-scoring-v3.js.map