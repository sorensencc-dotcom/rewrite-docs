 ...
 */
import { generateCicCostComputeReport } from '../report/CicCostComputeReport';
import { getAllRoutingSignals } from '../../orchestrator/routingCostSignals';
/**
 * Main skill function
 * Returns typed slices of the unified cost report
 */
export async function cicCostComputeSkill(input) {
    ...
    switch (input.query) {
        ...
        case 'routing':
            ...
            data = {
                signals,
                summary: signals.reduce((acc, sig) => {
                    acc.totalAgents += 1;
                    if (sig.localBias > 0)
                        acc.preferringLocal += 1;
                    return acc;
 ...
 */
export async function invokeSkill(query) {
    const result = await cicCostComputeSkill({ query });
    return result.data;
}
//# sourceMappingURL=cicCostComputeSkill.js.map