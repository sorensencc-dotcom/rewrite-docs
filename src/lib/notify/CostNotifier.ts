...
export class CostNotifier {
   ...
   */
  static async sendSlackDaily(report: CicCostComputeReport, period: 'daily' | 'weekly' = 'daily'): Promise<boolean> {
   ...
   */
  static async sendEmailDaily(report: CicCostComputeReport, period: 'daily' | 'weekly' = 'daily'): Promise<boolean> {
  ...
  private static formatEmailHtml(report: CicCostComputeReport, period: 'daily' | 'weekly'): string {
    const gpuCost = report.local?.gpuCostPerDay ?? 0;
    const savings = report.local?.dailySavings ?? 0;
