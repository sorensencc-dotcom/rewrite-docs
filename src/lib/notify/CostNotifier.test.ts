...
describe('CostNotifier', () => {
  const mockReport: CicCostComputeReport = {
    usage: {
      dailyTokens: 50000,
      weeklyTokens: 350000,
  ...
  describe('sendSlackDaily', () => {
    it('should handle missing webhook gracefully', async () => {
      ...
      if (oldEnv) process.env.CIC_SLACK_WEBHOOK_URL = oldEnv;
    });
    ...
    it('should include budget alert when triggered', () => {
      const alertReport: CicCostComputeReport = {
        ...mockReport,
        budget: { ema: 6.00, alert: true, limit: 5.00 },
      };

      const html = (CostNotifier as any).formatEmailHtml(alertReport, 'daily');
  ...
  describe('sendEmailDaily', () => {
    it('should handle missing email config gracefully', async () => {
      ...
      if (oldEnv) process.env.CIC_NOTIFY_EMAIL = oldEnv;
    });
  ...
  describe('formatEmailHtml', () => {
    it('should generate valid HTML structure', () => {
      const html = (CostNotifier as any).formatEmailHtml(mockReport, 'weekly');
    ...
    it('should handle missing agent data', () => {
      const reportNoAgents: CicCostComputeReport = {
        ...mockReport,
        agents: { burn: {}, savings: {} },
      };

      const html = (CostNotifier as any).formatEmailHtml(reportNoAgents, 'daily');
