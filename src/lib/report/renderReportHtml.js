 ...
 */
import { renderUsageCost7dSvg, renderLocalSavings7dSvg, renderPerModelCostSvg, } from '../charts/CostCharts';
/**
 * Generate full HTML report with dark theme + inline SVGs
 */
export function renderReportHtml(report, options = {}) {
    const { title = 'CIC Cost Compute Report', period = 'daily', includeCharts = true, } = options;
    ...
    const modelData = Object.entries(report.agents.burn).map(([agent, data]) => ({
        model: agent,
        cost: data.cost,
        tokens: data.tokens,
    }));
...
//# sourceMappingURL=renderReportHtml.js.map