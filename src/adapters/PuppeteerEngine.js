...
import { PuppeteerResultSchema } from '../validation/schemas';
    ...
    async run(script, options) {
        ...
        try {
            ...
            if (!parsed.success) {
                ...
                return makeError('PUPPETEER_INVALID_RESULT', { reason: 'schema validation failed', errors: parsed.error }, adapter, startTime);
            }
            ...
            if (detectCrashInLogs(parsed.data.logs)) {
                ...
                return makeError('PUPPETEER_CRASHED', { reason: 'crash markers detected in logs', logs: parsed.data.logs }, adapter, startTime);
            }
            ...
            if (!result.success) {
                ...
                return makeError('PUPPETEER_FAILED', { reason: 'Puppeteer execution failed', logs: result.logs }, adapter, startTime);
            }
            const durationMs = Date.now() - startTime;
            ...
            metricsExporter.recordAdapterCall(adapter, durationMs, 'error');
            metricsExporter.recordAdapterError(adapter, 'PUPPETEER_FAILED');
            return makeError('PUPPETEER_FAILED', { reason: err instanceof Error ? err.message : 'unknown error' }, adapter, startTime);
        }
    }
}
