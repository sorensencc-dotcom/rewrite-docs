    ...
    async run(url, timeout = 30000) {
        ...
        try {
            ...
            if (!parsed.success) {
                ...
                return makeError('INVALID_NAVIGATION_RESULT', { reason: 'schema validation failed', errors: parsed.error }, adapter, startTime);
            }
            ...
            if (!validateFinalUrl(parsed.data.url)) {
                ...
                return makeError('INVALID_URL', { reason: 'final URL rejected', url: parsed.data.url }, adapter, startTime);
            }
            const durationMs = Date.now() - startTime;
            ...
            return makeError('NAVIGATION_FAILED', { reason: err instanceof Error ? err.message : 'unknown error' }, adapter, startTime);
        }
    }
}
