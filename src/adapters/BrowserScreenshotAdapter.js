...
import { ScreenshotResultSchema } from '../validation/schemas';
    ...
    async run(options) {
        ...
        try {
            ...
            if (!parsed.success) {
                ...
                return makeError('INVALID_SCREENSHOT_RESULT', { reason: 'schema validation failed', errors: parsed.error }, adapter, startTime);
            }
            ...
            if (!validatePng(parsed.data.base64)) {
                ...
                return makeError('INVALID_IMAGE_FORMAT', { reason: 'PNG header validation failed' }, adapter, startTime);
            }
            // Guard: validate size
            if (!validateScreenshotSize(parsed.data.base64)) {
                ...
                return makeError('SCREENSHOT_TOO_LARGE', { reason: 'screenshot exceeds 5MB limit', size: parsed.data.base64.length }, adapter, startTime);
            }
            const durationMs = Date.now() - startTime;
            ...
            return makeError('SCREENSHOT_FAILED', { reason: err instanceof Error ? err.message : 'unknown error' }, adapter, startTime);
        }
    }
}
